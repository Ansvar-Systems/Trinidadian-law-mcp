#!/usr/bin/env tsx
/**
 * Drift detection for Trinidad and Tobago Law MCP.
 *
 * Checks if upstream laws.gov.tt content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Trinidad and Tobago Law MCP -- Drift Detection');
  console.log('================================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'trinidadian-law-mcp/1.0 drift-detect' },
      });
