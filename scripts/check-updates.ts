#!/usr/bin/env tsx
/**
 * Trinidad and Tobago Law MCP -- Data Freshness Checker
 *
...
const PORTAL_URL = 'https://laws.gov.tt';
const PORTAL_NAME = 'Digital Legislative Library of Trinidad and Tobago';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/trinidadian-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Trinidad and Tobago Law MCP -- Data Freshness Check');
