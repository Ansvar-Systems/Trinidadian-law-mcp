/**
 * Golden contract tests for Trinidad and Tobago Law MCP.
 *
...
describe(`Contract tests: ${fixture.mcp_name}`, () => {
  beforeAll(async () => {
    const dbPath =
      process.env['TT_LAW_DB_PATH'] ?? join(__dirname, '..', '..', 'data', 'database.db');
    ...
    const server = new Server(
      { name: 'trinidadian-law-test', version: '0.0.0' },
      { capabilities: { tools: {} } },
    );
